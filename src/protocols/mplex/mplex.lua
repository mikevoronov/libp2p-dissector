...
function mplex_proto.dissector(buffer, pinfo, tree)
    ...
    local subtree = tree:add(mplex_proto, "MPLEX protocol")
    ...
    local headerTree = subtree:add(buffer(4, headerSize), string.format("MPLEX header: uvarint decoded 0x%X", header))
    ...
    subtree:add(buffer(4 + headerSize, lenSize), string.format("MPLEX len: uvarint decoded 0x%X", len))
    subtree:add(buffer(4 + headerSize + lenSize, plain_text:len() - lenSize - headerSize), "content")
